...
import { defineSubscriptMarkdown } from './subscript'
import { remarkSubscript } from './remarkSubscript'

const extension = getMarksBaseExtensions([defineSubscriptMarkdown()])

const { doc, p, subscript } = builders(extension.schema!, {
...
test('markdown -> prosemirror', () => {
  ...
  const unist = unistNodeFromMarkdown(
    `Just a test content with <sub>subscript text</sub>`,
    {
      transformers: [{ type: 'remarkPlugin', handler: remarkSubscript }],
    },
  )
  ...
  sameNode(
    result,
    doc(p('Just a test content with ', subscript('subscript text'))),
  )
})
...
test('prosemirror -> markdown', () => {
  ...
  sameMarkdown(result, 'Just a test content with <sub>subscript text</sub>')
})
