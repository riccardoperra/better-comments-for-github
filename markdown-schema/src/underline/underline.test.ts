...
const extension = getMarksBaseExtensions([defineUnderlineMarkdown()])

const { doc, p, underline } = builders(extension.schema!, {
...
test('markdown -> prosemirror', () => {
  ...
  const unist = unistNodeFromMarkdown(
    `Just a test content with <ins>underlined text</ins>`,
    {
      transformers: [{ type: 'remarkPlugin', handler: remarkUnderline }],
    },
  )
  ...
  sameNode(
    result,
    doc(p('Just a test content with ', underline('underlined text'))),
  )
})
...
test('prosemirror -> markdown', () => {
  ...
  sameMarkdown(result, 'Just a test content with <ins>underlined text</ins>')
})
