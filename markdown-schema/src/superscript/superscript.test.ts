...
import { defineSuperscriptMarkdown } from './superscript'

const extension = getMarksBaseExtensions([defineSuperscriptMarkdown()])

const { doc, p, superscript } = builders(extension.schema!, {
...
test('markdown -> prosemirror', () => {
  ...
  const unist = unistNodeFromMarkdown(
    `Just a test content with <sup>superscript text</sup>`,
    {
      transformers: [{ type: 'remarkPlugin', handler: remarkSuperscript }],
    },
  )
  ...
  sameNode(
    result,
    doc(p('Just a test content with ', superscript('superscript text'))),
  )
})
...
test('prosemirror -> markdown', () => {
  ...
  sameMarkdown(result, 'Just a test content with <sup>superscript text</sup>')
})
